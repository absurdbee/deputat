...
class UserLoadPlaylist(ListView):
    ...
    def get(self,request,*args,**kwargs):
        ...
        if request.user.is_authenticated:
            ...
            self.template_name = get_template_anon_user_window(self.list, "user_music/load/anon_list.html", request.META['HTTP_USER_AGENT'])
...
class UserLoadModeratedPlaylist(ListView):
	...
	def get(self,request,*args,**kwargs):
		...
		self.list = SoundList.objects.get(pk=self.kwargs["pk"])
		self.template_name = get_managers_template("user_music/load/moderation_list.html", request.user, request.META['HTTP_USER_AGENT'])
