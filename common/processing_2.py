...
def get_formatted_text(text, is_message=False):
    _words = text.replace("<img src"," |<imgsrc").replace('.png">','.png">| ').replace("<br>"," <br>")
        ...
        for word in words:
            ...
            if "#" in word:
                ...
                if word[0] == "#":
                    _p = word.strip(".,:;!_*-+()/@#¤%&)")
                    tag = "#" + _p
                    _loop[next] = _loop[this].replace(indent + tag, indent + '<a class="ajax action"href="/search/?q=%23' + _p + '">#' + _p + '</a>', 1).replace("#<", "<")
                else:
                    _p = word.strip(".,:;!_*-+()/@#¤%&)")
                    p_2 = _p[_p.find("#") + 1:]
                    tag = "#" + p_2
                    _loop[next] = _loop[this].replace(tag, '<a class="ajax action"href="/search/?q=%23' + p_2 + '">#' + p_2 + '</a>', 1)
            if "." in word:
                _p = word.strip(".,:;!_*-+()/@#¤%&)")
                if "." in _p and not ".png" in _p:
                    if _p[0] == "h":
                        ...
                        p_2 = "//" + _p
                    if "служународу.рус" in _p:
                        _loop.append("")
                        p_2 = _p.replace("служународу.рус", "/").replace("http://", "").replace("https://", "")
                        this += 1
                        next += 1
                        _loop[next] = _loop[this].replace(indent + _p, indent + '<a class="ajax action"href="' + p_2 + '">' + _p[:30] + '</a>', 1)
                    else:
                        p_items = _p.split(".")
                        try:
                        ...
                        for zone in zons:
                            if zone == p_zone:
                                ...
                                this += 1
                                next += 1
                                _loop[next] = _loop[this].replace(_p, '<a class="action"rel="nofollow"target="_blank"href="' + p_2 + '">' + _p[:30] + '</a>', 1)
                                break
                    _exlude.append(_p)
