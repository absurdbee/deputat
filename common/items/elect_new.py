...
def wall_elect_new(user, elect_new):
    ...
    if user.is_anonymous:
        ...
        if user.is_supermanager():
            card_drop += '<span><span class="dropdown-item u_elect_new_remove">Удалить</span></span><span class="dropdown-item u_edit_elect_new">Редактировать</span><span><span class="dropdown-item u_elect_new_fixed">Закрепить</span></span>'
        else:
    ...
    for tag in elect_new.get_manager_tags():
        tags += '<a class="ajax" href="/search/?tag_name=' + tag.name + '">' + tag.name + '</a>'

    if elect_new.elect:
        elect = elect_new.elect
    else:
        elect = Elect.objects.get(pk=62)
    _description = elect_new.description[:180]
    TAG_RE = re.compile(r'<[^>]+>')
    description = TAG_RE.sub('', _description)
    ...
    ' + elect_new.get_created() + ' | ' + tags + '</span></h4><div class="card-text event_description"><span><span class="elect_new_window pointer">' + description + '...</span></span>\
    <br><br><a class="mt-1 ajax" style="text-decoration:underline" href="/elect/' + str(elect.pk) + '">' + elect.name + '</a><div class="" style="padding: 5px;position: absolute;bottom: 3px;width: 100%;">\
