...
class OrganizationComment(models.Model):
    ...
    parent = models.ForeignKey('self', on_delete=models.SET_NULL, related_name='organization_comment_replies', null=True, blank=True, verbose_name="Родительский комментарий")
    created = models.DateTimeField(auto_now_add=True, auto_now=False, verbose_name="Создан")
    commenter = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name="Комментатор")
    text = models.TextField(blank=True)
    ...
    def get_edit_attach(self, user):
        from common.attach.comment_attach import get_organization_comment_edit
        return get_organization_comment_edit(self, user)
    ...
    def get_replies(self):
        return self.organization_comment_replies.filter(Q(type="PUB")|Q(type="EDI"))

    def count_replies(self):
        return self.get_replies().values("pk").count()

    def likes(self):
        from common.model.votes import OrganizationCommentVotes
        return OrganizationCommentVotes.objects.filter(comment_id=self.pk, vote__gt=0)
    ...
    def create_comment(cls, commenter, attach, new, parent, text):
        from common.notify.notify import user_comment_notify, user_comment_wall
        from common.processing import get_organization_comment_processing
        ...
        _attach = str(attach)
        _attach = _attach.replace("'", "").replace("[", "").replace("]", "").replace(" ", "")
        comment = cls.objects.create(commenter=commenter, attach=_attach, parent=parent, new=new, text=text)
        new.comment += 1
        new.save(update_fields=["comment"])
        commenter.plus_comments(1)
        if comment.parent:
    ...
    def edit_comment(self, attach, text):
        ...
        self.attach = _attach
        self.text = text
        self.type = OrganizationComment.EDITED
        return self.save()
    ...
    def get_u_attach(self, user):
        from common.attach.comment_attach import get_u_organization_comment_attach
        return get_u_organization_comment_attach(self, user)
    ...
    def delete_comment(self):
        from notify.models import Notify, Wall
        if self.type == "PUB":
            self.type = OrganizationComment.DELETED
        elif self.type == "EDI":
            self.type = OrganizationComment.DELETED
        self.save(update_fields=['type'])
        if self.parent:
            self.parent.organization.comment -= 1
            self.parent.organization.save(update_fields=["comment"])
            if Notify.objects.filter(type="ORGC", object_id=self.pk, verb__contains="REP").exists():
                Notify.objects.filter(type="ORGC", object_id=self.pk, verb__contains="REP").update(status="C")
        else:
            self.organization.comment -= 1
    ...
    def restore_comment(self):
        ...
        if self.parent:
            ...
            self.parent.organization.save(update_fields=["comment"])
            if Notify.objects.filter(type="ORGC", object_id=self.pk, verb__contains="REP").exists():
                Notify.objects.filter(type="ORGC", object_id=self.pk, verb__contains="REP").update(status="R")
        else:
            self.organization.comment += 1
            self.organization.save(update_fields=["comment"])
            if Notify.objects.filter(type="ORGC", object_id=self.pk, verb__contains="COM").exists():
    ...
    def close_item(self):
        from notify.models import Notify, Wall
        if self.type == "PUB":
            self.type = OrganizationComment.CLOSED
        elif self.type == "EDI":
            self.type = OrganizationComment.EDITED_CLOSED
        self.save(update_fields=['type'])
        if self.parent:
            self.parent.organization.comment -= 1
            self.parent.organization.save(update_fields=["comment"])
    ...
    def send_like(self, user, community):
        ...
        from common.notify.notify import user_notify, user_wall
        try:
            item = OrganizationCommentVotes.objects.get(comment=self, user=user)
            item.delete()
            self.like -= 1
            self.save(update_fields=['like'])
        except OrganizationCommentVotes.DoesNotExist:
            OrganizationCommentVotes.objects.create(comment=self, user=user, vote=OrganizationCommentVotes.LIKE)
            self.like += 1
            self.save(update_fields=['like'])
            if self.parent:
                if community:
                    ...
                    from common.notify.notify import user_notify, user_wall
                    user_notify(user, None, self.pk, "ORGC", "u_organization_comment_notify", "LRE")
                ...
                if community:
                    ...
                    from common.notify.notify import user_notify, user_wall
                    user_notify(user, None, self.pk, "ORGC", "u_organization_comment_notify", "LCO")
                    user_wall(user, None, self.pk, "ORGC", "u_organization_comment_notify", "LCO")
        return HttpResponse(json.dumps({"like_count": str(self.likes_count())}),content_type="application/json")
