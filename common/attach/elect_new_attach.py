def get_elect_new_attach(post, user):
    block = ''
    for item in post.attach.split(","):
        if item[:3] == "pho":
            try:
                ...
                block = ''.join([block, '<div class="photo"><div class="progressive replace image_fit elect_new_photo pointer" data-href="', photo.file.url, '" photo-pk="', str(photo.pk), '"><img class="preview image_fit" width="20" height="15" loading="lazy" src="', photo.preview.url,'" alt="img"></div></div>'])
            except:
                pass
        elif item[:3] == "vid":
            try:
                ...
                block = ''.join([block, '<div class="video"><img class="image_fit" src="', video.image.url, '" alt="img"><div class="video_icon_play_v2 elect_new_video" video-pk="', str(video.pk), '" data-uuid="', str(video.uuid), '" video-counter="0"></div></div>'])
            except:
                pass
        elif item[:3] == "mus":
            try:
