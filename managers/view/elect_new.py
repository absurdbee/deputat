...
class CreateManagerElect(TemplateView):
    ...
    def post(self,request,*args,**kwargs):
        ...
        if request.is_ajax() and self.form_post.is_valid() and request.user.is_elect_new_manager():
            post = self.form_post.save(commit=False)
            new_post = post.create_elect(creator=request.user, name=post.name, description=post.description, image=post.image,  list=request.POST.getlist("list"), region=request.POST.getlist("region"), city=request.POST.getlist("city"),birthday=post.birthday, fraction=post.fraction, post=post.post)
            return HttpResponse()
        else:
            from django.http import HttpResponseBadRequest
            return HttpResponseBadRequest()
...
class EditManagerElect(TemplateView):
    ...
    def post(self,request,*args,**kwargs):
        ...
        if request.is_ajax() and self.form_post.is_valid() and request.user.is_elect_new_manager():
            post = self.form_post.save(commit=False)
            new_post = post.edit_elect(name=post.name, description=post.description, image=post.image, list=request.POST.getlist("list"), region=request.POST.getlist("region"), city=request.POST.getlist("city"),birthday=post.birthday, fraction=post.fraction, manager_id=request.user.pk, post=post.post)
            return HttpResponse()
        else:
            from django.http import HttpResponseBadRequest
            return HttpResponseBadRequest()
