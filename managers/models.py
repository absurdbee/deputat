...
class Moderated(models.Model):
    ...
    def get_user(self):
        try:
            ...
            str(self.object_id), '" class="ajax"><h6 class="mt-1">', user.get_full_name(), '</h6></a><span class="mt-1 pointer underline show_object_reports">', self.reports_count_ru(), \
...
class ModerationReport(models.Model):
    ...
    def is_user_already_reported(cls, reporter_id, type, object_id):
        return cls.objects.filter(reporter_id=reporter_id, type=moderated_type, moderated_object_id=object_id).exists()
...
class ModerationPenalty(models.Model):
    ...
    def get_music(self):
        try:
            ...
            opt_drop = '<div class="dropdown" style="position: inherit;"><a class="btn_default drop pointer"><svg style="width: 17px;padding-top:3px" fill="currentColor" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"></path><path d="M12 8c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm0 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z"></path></svg></a><div class="dropdown-menu dropdown-menu-right" style="top: 60px;">' + options + '</div></div>'
            span_btn = ''.join([span_btn, '<span class="span_btn">', opt_drop, '</span>'])
