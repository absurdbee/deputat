...
class Chat(models.Model):
        ...
        if self.attach:
            ...
            for i in self.attach.split(","):
                if i[:3] == "doc":
                    ...
                    list.append(DocList.objects.get(pk=i[3:]))
            return list
        return []
        ...
        if self.attach:
            ...
            for i in self.attach.split(","):
                if i[:3] == "mus":
                    ...
                    list.append(SoundList.objects.get(pk=i[3:]))
            return list
        return []
    ...
    def get_preview_message(self, user_id):
        ...
        if self.is_group() or self.is_public():
            ...
            media_body = ''.join(['<div class="media-body" style="margin-left:5px"><h5 class="time-title mb-0">', chat_name, request_chat_user.get_beep_icon(), '<small class="float-right text-muted">', str(created), '</small></h5><p class="mb-0', is_read ,'" style="white-space: nowrap;">', preview_text, '</p><span class="typed"></span></div>'])
            ...
            if member.get_online():
                ...
                status = ''
            media_body = ''.join(['<div class="media-body" style="margin-left:5px"><h5 class="time-title mb-0">', chat_name, request_chat_user.get_beep_icon(), status, '<small class="float-right text-muted">', str(created), '</small></h5><p class="mb-0', is_read ,'" style="white-space: nowrap;">', preview_text, '</p><span class="typed"></span></div>'])
            return ''.join(['<div class="media">', figure, media_body, self.get_unread_count_message(user_id), '</div>'])
    ...
    def get_header_chat(self, user_id):
        ...
        dropdown = ''.join(['<div class="dropdown d-inline-block"><a style="cursor:pointer;padding-right: 40px;" class="icon-circle icon-30 btn_default drop"><svg class="svg_info" fill="currentColor" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"/><path d="M12 8c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm0 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z"/></svg></a><div class="dropdown-menu dropdown-menu-right"><a class="dropdown-item chat_search pointer">Поиск сообщений</a><a class="dropdown-item show_attach_files pointer">Показать вложения</a>', muted_drop, dop_drops,'<a class="dropdown-item u_clean_chat_messages pointer">Очистить историю</a></div></div>'])
...
class Message(models.Model):
    ...
    def get_draft_transfers_block(self):
        ...
        return '<div><span>' + text_2 + '</span><div style="position:relative;padding-bottom:7px"><div><span class="pointer underline">' + text + '</span><span class="remove_parent_block pointer message_form_parent_block">x</span></div></div>' + inputs + '</div>'
