...
class Moderated(models.Model):
    ...
    def get_photo(self):
        try:
            ...
            return ''.join(['<div class="uuid_keeper" data-uuid="', str(photo.uuid), '"><div class="progressive replace image_fit_200 u_photo_moderated_detail pointer" data-href="', photo.file.url, '" photo-pk="', str(photo.pk), '"><img class="preview uuid_keeper image_fit" width="20" height="15" loading="lazy" src="', photo.preview.url,'" alt="img"></div></div>'])
        except:
            return ''
    def get_video(self):
        try:
