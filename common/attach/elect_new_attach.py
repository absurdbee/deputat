    ...
    for item in comment.attach.split(","):
        if item[:3] == "pho":
            try:
                ...
                block = ''.join([block, '<div class="row_item"><div class="progressive replace image_fit_200 u_blog_photo pointer" data-href="', photo.file.url, '" photo-pk="', str(photo.pk), '"><img class="preview image_fit" width="20" height="15" loading="lazy" src="', photo.preview.url,'" alt="img"></div></div>'])
            except:
                pass
        elif item[:3] == "vid":
            try:
            ...
            try:
                ...
                block = ''.join([block, '<div class="col-md-12 pt-1" style="flex-basis: 100%;"><div class="media border p-1"><div class="media-body doc_media_body" style="line-height: 8px;"><span>', music.title, '</span><div class="audio_div"><audio id="player" class="audio_player"><source src="', music.get_uri(), '" type="audio/mp3" /></audio></div>', span_btn, '</div></div></div>'])
            except:
                pass
        elif item[:3] == "doc":
            try:
                ...
                if user.is_authenticated:
                    ...
                    opt_drop = '<div class="dropdown" style="position: inherit;"><a class="btn_default drop pointer"><svg class="svg_info" style="padding-top: 3px;" fill="currentColor" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"></path><path d="M12 8c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm0 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z"></path></svg></a><div class="dropdown-menu dropdown-menu-right" style="top: 48px;">' + options + '<span class="dropdown-item copy_link">Копировать ссылку</span></div></div>'
            ...
            block = ''.join([block, '<div style="flex-basis: 100%;" class="border mt-1"><div class="card-body" playlist-pk="', str(playlist.pk), '"style="padding: 8px;padding-bottom: 0;"><div style="display:flex"><figure><a class="u_load_playlist pointer">', image, '</a></figure><div class="media-body" style="margin-left: 10px;"><h6 class="my-0 mt-1 u_load_playlist pointer">', playlist.name, '</h6><p>Плейлист <a class="ajax underline" href="/users/', str(creator.pk), '">', str(creator.get_full_name_genitive()), '</a><br>Треков: ', str(playlist.count_items()), '</p></div><span class="list_share">', add_svg, '</span></div></div></div>'])
            #except:
            #    pass
        elif item[:3] == "ldo":
            try:
                ...
                block = ''.join([block, '<div style="flex-basis: 100%;" class="mt-1"><div class="card-body border" doclist-pk="', str(list.pk), '" style="padding-bottom: 0;"><div style="display:flex"><figure><a class="u_load_doc_list pointer">', image, '</a></figure><div class="media-body" style="margin-left: 10px;"><h6 class="my-0 mt-1 u_load_doc_list pointer">', list.name, '</h6><p>Список документов <a class="ajax underline" href="/users/', str(creator.pk), '">', str(creator.get_full_name_genitive()), '</a><br>Документов: ', str(list.count_items()), '</p></div><span class="list_share">', add_svg, '</span></div></div></div>'])
            except:
                pass
        elif item[:3] == "lph":
            try:
    ...
    for item in comment.attach.split(","):
        if item[:3] == "pho":
            try:
                from gallery.models import Photo
                photo = Photo.objects.get(query, pk=item[3:])
                block = ''.join([block, '<div class="row_item"><div class="progressive replace image_fit u_elect_new_photo pointer" data-href="', photo.file.url, '" photo-pk="', str(photo.pk), '"><img class="preview image_fit" width="20" height="15" loading="lazy" src="', photo.preview.url,'" alt="img"></div></div>'])
            except:
                pass
        elif item[:3] == "vid":
            try:
                ...
                if user.is_authenticated:
                    ...
                    for list in user.get_doc_lists():
                        if list.is_item_in_list(doc.pk):
                            ...
                            lists = ''.join([lists, '<span data-uuid="', str(list.uuid), '"><span class="dropdown-item u_add_doc_in_list" style="padding-left: 30px;">', list.name, '</span></span>'])
                    span_btn = ''.join([span_btn, '<span class="span_btn" data-pk="', str(doc.pk), '"><span class="dropdown" style="position: inherit;" data-pk="', str(doc.pk), '"><span class="btn_default pointer drop" title="Добавить в плейлист"><svg fill="currentColor" style="width:25px;height:25px;margin-top: 2px;" class="svg_default" viewBox="0 0 24 24"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/><path d="M0 0h24v24H0z" fill="none"/></svg></span><div class="dropdown-menu dropdown-menu-right" style="top: 48px;">', lists, '</div></span>', opt_drop, '</span>'])
                block = ''.join([block, '<div class="col-md-12 pt-1" style="flex-basis: 100%;"><div class="media border"><svg fill="currentColor" class="svg_default" style="width:45px;margin: 0;" viewBox="0 0 24 24"><path d="M0 0h24v24H0z" fill="none"/><path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/></svg><div class="media-body doc_media_body"><h6 class="pointer" style="width: 84%;overflow: hidden;padding-top: 5px;"><a href="', doc.file.url, '" target="_blank" rel="nofollow">', doc.title, '</a></h6><span class="small" style="bottom: 5px;position: absolute;">', str(doc.file.size), ' | ', doc.get_mime_type(), '</span>', span_btn, '</div></div></div>'])
            except:
                pass
        elif item[:3] == "sur":
            try:
            ...
            try:
                ...
                block = ''.join([block, '<div class=" mt-1" style="flex-basis: 100%;"><div class="card-body border" doclist-pk="', str(list.pk), '" style="padding-bottom: 0;"><div style="display:flex"><figure><a class="u_load_doc_list pointer">', image, '</a></figure><div class="media-body" style="margin-left: 10px;"><h6 class="my-0 mt-1 u_load_doc_list pointer">', list.name, '</h6><p>Список документов <a class="ajax underline" href="/users/', str(creator.pk), '">', str(creator.get_full_name_genitive()), '</a><br>Документов: ', str(list.count_items()), '</p></div><span class="list_share">', add_svg, '</span></div></div></div>'])
            except:
                pass
        elif item[:3] == "lph":
            try:
...
def get_elect_new_edit(new, user):
    block = ''
    for item in new.attach.split(","):
        if item[:3] == "pho":
            ...
            try:
                from music.models import Music
                music = Music.objects.get(query, pk=item[3:])
                block = ''.join([block, '<div class="border" style="position:relative; width:100%; padding:10px;padding-right:35px;margin-bottom:7px"><span class="music_preview_delete" tooltip="Не прикреплять" flow="up"><svg fill="#FF0000" viewBox="0 0 24 24"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"></path><path d="M0 0h24v24H0z" fill="none"></path></svg></span><input type="hidden" name="attach_items" value="mus', track.pk, '"><div class="media-body music_media_body" style="lihe-heigh:7px"><span class="title">', track.title, '</span><audio class="audio_player"><source src="', track.file.url, '" type="audio/mp3" /></audio><div></div>'])
            except:
                pass
        elif item[:3] == "doc":
            try:
