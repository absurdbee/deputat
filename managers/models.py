...
class ModerationPenalty(models.Model):
    ...
    def get_user(self, user_id):
        ...
        user = User.objects.get(pk=user_id)
        if self.is_suspend():
            span = '<span class="small">До ', self.expiration, '</span>(<a class="small remove_user_suspend pointer">Отменить заморозку</a> |<a class="small user_unverify pointer">Отменить проверку</a>)'
        elif self.is_closed():
