...
class ModerationPenalty(models.Model):
    ...
    def get_photo_list(self):
        ...
        add = ''
        block = ''.join(['<div class="text-center bg-dark position-relative big_mobile_element col-md-6" photolist-pk="', str(self.object_id), '"><figure class="background-img"><img src="', list.get_cover_photo(), '">"</figure><div class="container p-3"><h4 class="u_load_manager_photo_list text-white pointer"><a class="nowrap">', list.name, '</a></h4><p><a class="ajax underline text-white nowrap" href="/users/">', str(creator), '</a></p><hr class="my-3"><a class="u_load_manager_photo_list text-white pointer">', list.count_items_ru(), '</a><div class="row">', add, '</div>', '</div></div>'])
        #except:
        #    pass
