...
def blog(user, blog):
    ...
    if user.is_anonymous:
        ...
        if user.is_supermanager():
            card_drop += '<span class="dropdown-item u_close_blog">Закрыть</span><span class="dropdown-item u_edit_blog">Редактировать</span>'
        else:
    ...
    <span><img class="img-fluid card-img-top blog_window pointer" src="' + blog.get_image() + '" alt="img"></span></span></div><div class="card-body event_body">\
    ...
    <span class="repost_count margin_right_5">', str(blog.count_reposts()), '</span></span><div class="dropdown-menu" style="top: -195px;" data-pk="', str(object.pk), '" data-link="/blog/', object.slug,'" data-title="', object.title,'"><span class="dropdown-header" style="font-weight:bold">Поделиться</span><span class="dropdown-item blog_share_vkontakte">VKontakte</span><span class="dropdown-item blog_share_facebook">Facebook</span><span class="dropdown-item blog_share_twitter">Twitter</span><span class="dropdown-item blog_share_telegram">Telegram</span></div><span ', comments_enabled, '  title="Комментарий" class="btn_default" style="cursor:pointer;margin-right: 5px;">\
