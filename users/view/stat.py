...
class UserCoberturaYear(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		self.views, self.years, self.template_name, pk = [], UserNumbers.objects.dates('created', 'year')[0:10], get_my_template("profile_stat/cobertura_year.html", request.user, request.META['HTTP_USER_AGENT']), request.user.pk
		for i in self.years:
...
class UserCoberturaMonth(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		self.views, self.months, self.template_name, pk = [], UserNumbers.objects.dates('created', 'month')[0:10], get_my_template("profile_stat/cobertura_month.html", request.user, request.META['HTTP_USER_AGENT']), request.user.pk
...
class UserCoberturaWeek(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		...
		self.range, self.views, self.weeks, self.template_name, pk = [], [], UserNumbers.objects.dates('created', 'week')[0:10], get_my_template("profile_stat/cobertura_week.html", request.user, request.META['HTTP_USER_AGENT']), request.user.pk
		for i in self.weeks:
...
class UserCoberturaDay(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		self.views, self.days, self.template_name, pk = [], UserNumbers.objects.dates('created', 'day')[0:10], get_my_template("profile_stat/cobertura_day.html", request.user, request.META['HTTP_USER_AGENT']), request.user.pk
		for i in self.days:
...
class UserTrafficYear(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		self.views, self.un_views, self.years, self.template_name, pk = [], [], UserNumbers.objects.dates('created', 'year')[0:10], get_my_template("profile_stat/traffic_year.html", request.user, request.META['HTTP_USER_AGENT']), request.user.pk
		for i in self.years:
...
class UserTrafficMonth(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		self.views, self.un_views, self.months, self.template_name, pk = [], [], [], UserNumbers.objects.dates('created', 'month')[0:10], get_my_template("profile_stat/traffic_month.html", request.user, request.META['HTTP_USER_AGENT']), request.user.pk
		for i in self.months:
...
class UserTrafficWeek(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		...
		self.views, self.range, self.un_views, self.weeks, self.template_name, pk = [], [], [], UserNumbers.objects.dates('created', 'week')[0:10], get_my_template("profile_stat/traffic_week.html", request.user, request.META['HTTP_USER_AGENT']), request.user.pk
		for i in self.weeks:
...
class UserTrafficDay(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		self.views, self.un_views, self.days, self.template_name, pk = [], [], UserNumbers.objects.dates('created', 'day')[0:10], get_my_template("profile_stat/traffic_day.html", request.user, request.META['HTTP_USER_AGENT']), request.user.pk
		for i in self.days:
