...
class PhoneSend(View):
    def post(self,request,*args,**kwargs):
        ...
        if not request.is_ajax() and not request.user.is_no_phone_verified():
            ...
            if len(_phone) > 8:
                phone = request.POST.get('first_number') + _phone
                try:
                    ...
                    response = requests.get(url="https://api.ucaller.ru/v1.0/initCall?service_id=12203&key=GhfrKn0XKAmA1oVnyEzOnMI5uBnFN4ck&phone=" + int(phone))
                    data = response.json()
