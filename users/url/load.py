...
urlpatterns = [
    ...
    url(r'^u_album_img_load/(?P<uuid>[0-9a-f-]+)/$', UserLoadPhotoAlbum.as_view(), name="u_photo_list_load"),

    url(r'^u_img_comment_load/$', UserLoadPhotoComment.as_view(), name="u_photo_comment_load"),
    url(r'^u_album_img_comment_load/(?P<uuid>[0-9a-f-]+)/$', UserLoadPhotoAlbumComment.as_view(), name="u_photo_list_comment_load"),

    url(r'^u_video_load/$', UserLoadVideo.as_view(), name="u_video_load"),
    url(r'^u_video_album_load/(?P<uuid>[0-9a-f-]+)/$', UserLoadVideoAlbum.as_view(), name="u_video_list_load"),
