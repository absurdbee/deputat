...
class VideoList(models.Model):
    ...
    def create_list(cls, creator, name, description, order, community, is_public):
        ...
        if not VideoList.is_user_can_added_list(creator.pk):
            pass
        if not order:
            order = 1
        if community:
    ...
    def is_not_empty(self):
        return self.video_list.filter(list=self).exclude(type__contains="_").exists()
    ...
    def get_user_lists_not_empty(cls, user_pk):
        ...
        result, lists = [] , cls.objects.filter(query)
        for list in lists:
    ...
    def is_user_can_added_list(cls, user_pk):
        from django.conf import settings
        return cls.get_user_lists_count(user_pk) <= settings.USER_MAX_VIDEO_LISTS
...
class Video(models.Model):
    ...
    media_list = models.ManyToManyField("lists.MediaList", related_name='video_media_list', blank=True, verbose_name="Медиа-список")
