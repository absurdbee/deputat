...
class PublishElectNew(TemplateView):
    ...
    def post(self,request,*args,**kwargs):
        ...
        if request.is_ajax() and self.form_post.is_valid() and request.user.is_elect_new_manager():
            ...
            new_post = post.make_publish_new(title=post.title, description=post.description, elect=request.POST.get("elect"), attach=request.POST.getlist("attach_items"), category=post.category, tags=request.POST.getlist("tags"), manager_id=request.user.pk, comments_enabled=post.comments_enabled, votes_on=post.votes_on)
            return HttpResponse()
        else:
...
class CreateManagerElect(TemplateView):
    ...
    def post(self,request,*args,**kwargs):
        ...
        if request.is_ajax() and self.form_post.is_valid() and request.user.is_elect_new_manager():
            post = self.form_post.save(commit=False)
            new_post = post.create_elect(creator=request.user, name=post.name, description=post.description, image=post.image,  list=request.POST.getlist("list"), region=request.POST.getlist("region"), area=request.POST.getlist("area"), birthday=post.birthday, fraction=post.fraction, post_2=post.post_2)
            return HttpResponse()
        else:
            from django.http import HttpResponseBadRequest
            return HttpResponseBadRequest()
...
class EditManagerElect(TemplateView):
    ...
    def post(self,request,*args,**kwargs):
        ...
        if request.is_ajax() and self.form_post.is_valid() and request.user.is_elect_new_manager():
            post = self.form_post.save(commit=False)
            new_post = post.edit_elect(name=post.name, description=post.description, image=post.image, list=request.POST.getlist("list"), region=request.POST.getlist("region"), area=request.POST.getlist("area"), birthday=post.birthday, fraction=post.fraction, manager_id=request.user.pk, post_2=post.post_2)
            return HttpResponse()
        else:
            from django.http import HttpResponseBadRequest
            return HttpResponseBadRequest()
