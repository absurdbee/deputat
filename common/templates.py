...
def get_fine_user(user):
    if user.is_suspended():
        ...
        return "generic/u_template/deleted_user.html"
    elif user.is_closed():
        return "generic/u_template/closed_user.html"
...
def get_template_user_item(item, folder, template, request_user, user_agent, staff):
    ...
    if request_user.type[0] == "_":
        ...
        return get_fine_user_item(request_user, item, folder, template)
...
def get_template_anon_user_item(item, folder, template, user_agent):
    ...
    if user.type[0] == "_":
        ...
        return get_folder(user_agent) + folder + "anon_" + template
