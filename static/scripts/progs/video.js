...
on('body', 'click', '.u_add_video_in_list', function() {
  ...
  link.open( 'GET', '/video/user_progs/add_video_in_list/' + pk + "/" + uuid + "/", true );
  link.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
  link.onreadystatechange = function () {
  if ( link.readyState == 4 && link.status == 200 ) {
    list = parent.querySelector(".u_add_video_in_list");
    list.style.paddingLeft = "14px";
    list.classList.add("u_remove_video_in_list");
    list.classList.remove("u_add_video_in_list");
...
on('body', 'click', '.u_remove_video_in_list', function() {
  ...
  pk = parent.parentElement.parentElement.parentElement.getAttribute("data-pk");
  link = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject( 'Microsoft.XMLHTTP' );
  link.open( 'GET', '/docs/user_progs/remove_video_in_list/' + pk + "/" + uuid + "/", true );
  link.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
  link.onreadystatechange = function () {
  if ( link.readyState == 4 && link.status == 200 ) {
    list = parent.querySelector(".u_remove_video_in_list");
    list.style.paddingLeft = "30px";
    list.classList.add("u_add_video_in_list");
    list.classList.remove("u_remove_video_in_list");
