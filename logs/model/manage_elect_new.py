...
class ElectNewManageLog(models.Model):
    LIST_CLOSED, ITEM_CREATED, ITEM_CLOSED, ITEM_PUBLISHED, ITEM_EDITED, COMMENT_CLOSED, SUGGEST_ITEM_REJECT = 'LCLO','ICRE','ICLO','IPUB','IEDI','CCLO','SIREJ'
    ...
    def get_item_preview(self):
        try:
            if "I" in self.action_type:
                ...
                return '<span data-pk="' + str(new.pk) + '"><span><span><span><span class="elect_new_window pointer underline" style="font-weight: bold;">' +  new.title + '</span></span></span></span></span>'
            else:
                from common.model.comments import ElectNewComment
                comment = ElectNewComment.objects.get(pk=self.item)
                return '"' + comment.text[:25] + '" к активности ' + '<span data-pk="' + str(comment.new.pk) + '"><span><span><span><span class="elect_new_window pointer underline" style="font-weight: bold;">' +  comment.new.title + '</span></span></span></span></span>'
        except:
            return "Ошибка"
