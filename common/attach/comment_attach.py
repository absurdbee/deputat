...
def get_u_blog_comment_attach(comment, user):
    ...
    for item in comment.attach.split(","):
        if item[:3] == "pho":
            ...
            block = ''.join([block, '<div style="flex-basis: 100%;" class="card border"><div class="card-body" playlist-pk="', str(playlist.pk), '"style="padding: 8px;padding-bottom: 0;"><div style="display:flex"><figure><a class="u_load_playlist pointer">', image, '</a></figure><div class="media-body" style="margin-left: 10px;"><h6 class="my-0 mt-1 u_load_playlist pointer">', playlist.name, '</h6><p>Плейлист <a class="ajax underline" href="/users/', str(creator.pk), '">', str(creator.get_full_name_genitive()), '</a><br>Треков: ', str(playlist.count_tracks()), '</p></div><span class="list_share">', add_svg, '</span></div></div></div>'])
            #except:
            #    pass
        elif item[:3] == "ldo":
            try:
