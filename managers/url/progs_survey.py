...
urlpatterns = [
    ...
    url(r'^delete_close/(?P<uuid>[0-9a-f-]+)/$', login_required(SurveyCloseDelete.as_view())),
    url(r'^create_rejected/(?P<uuid>[0-9a-f-]+)/$', login_required(SurveyRejectedCreate.as_view())),
    url(r'^create_claim/(?P<uuid>[0-9a-f-]+)/$', login_required(SurveyClaimCreate.as_view())),
    url(r'^unverify/(?P<photo_uuid>[0-9a-f-]+)/(?P<obj_pk>\d+)/$', login_required(SurveyUnverify.as_view())),
]
