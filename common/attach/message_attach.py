    ...
    for item in message.attach.split(","):
        if item[:3] == "pho":
            ...
            if photo.type[0] == "_":
                pass
            if photo.community:
                el = ''.join(['<div class="image_fit_200 c_message_photo pointer" photo-pk="', str(photo.pk), '"><img class="image_fit"  src="', photo.file.url,'" alt="img"></div>'])
            else:
                el = ''.join(['<div class="image_fit_200 u_message_photo pointer" photo-pk="', str(photo.pk), '"><img class="image_fit" src="', photo.file.url,'" alt="img"></div>'])
            block = ''.join([block, '<div class="col-md-4" style="padding:5px"><span class="photo_preview_delete" tooltip="Не прикреплять" flow="up"><svg fill="#FF0000" viewBox="0 0 24 24"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"></path><path d="M0 0h24v24H0z" fill="none"></path></svg></span><span><input type="hidden" name="attach_items" value="pho', str(photo.pk), '"></span>', el, '</div>'])
            #except:
            #    pass
        elif item[:3] == "vid":
            try:
