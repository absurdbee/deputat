...
class UserLoadPenaltySurveyList(ListView):
	...
	def get(self,request,*args,**kwargs):
		...
		self.list = SurveyList.objects.get(pk=self.kwargs["pk"])
		self.template_name = get_managers_template(self.list, "user_survey/load/", "penalty_list.html", request.user, request.META['HTTP_USER_AGENT'], request.user.is_doc_manager())
...
class UserLoadModeratedSurveyList(ListView):
	...
	def get(self,request,*args,**kwargs):
		...
		self.list = SurveyList.objects.get(pk=self.kwargs["pk"])
		self.template_name = get_managers_template(self.list, "user_survey/load/", "moderated_list.html", request.user, request.META['HTTP_USER_AGENT'], request.user.is_doc_manager())
